...
function UploadMultipleData()
{
	...
	/* $_FILES mulit-dimanesional array structered as:
			[formfieldname] => array(
				[name] => array(  )
					[0] => string()"filename.extension"
				[type] => array(  )
					[0] => string()"mime/type"
				[tmp_name] => array(  )
					[0] => string()"/tmp/random_path"
				[error] => array(  )
					[0] => int(error_code)
				[size] => array(  )
					[0] => int(size_in_bytes)
	...
	{ // $key is equivalant to [i] in c++, $error is current value of ["error"]
		...
		if($error == UPLOAD_ERR_OK)
		{
			...
			if( !in_array($fileExtension, $allowedTypes) )
			{
				...
				{// tell user what the supported file types are
...
$(document).ready(function()
{
	...
	$("#dlBTN").click(function()
	{
		...
		{// 1 or more files ready to put in URL
			...
			$("#dlBTN").attr("onclick", document.location.href="downloadfiles.php?" + fileIdString);
		}
	});
});
